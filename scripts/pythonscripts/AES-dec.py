Nk = 0
Nb = 4
Nr = 0
state = [[0 for x in range(4)] for y in range(4)]
out = []
inn = []
RoundKey = []
Key = []


def getSBoxInvert(num):
    rsbox = []
    return rsbox[num]


def getSBoxValue(num):
    sbox = []
    return sbox[num]


Rcon = []


def KeyExpansion():
    temp = [0 for y in range(4)]
    for i in range(Nk):
        RoundKey.append(Key[i * 4])
        RoundKey.append(Key[i * 4 + 1])
        RoundKey.append(Key[i * 4 + 2])
        RoundKey.append(Key[i * 4 + 3])

    i = Nk
    while (i < (Nb * (Nr + 1))):
        for j in range(4):
            temp[j] = (RoundKey[(i - 1) * 4 + j])
        if (i % Nk == 0):
            k = temp[0]
            temp[0] = temp[1]
            temp[1] = temp[2]
            temp[2] = temp[3]
            temp[3] = k
            temp[0] = getSBoxValue(temp[0])
            temp[1] = getSBoxValue(temp[1])
            temp[2] = getSBoxValue(temp[2])
            temp[3] = getSBoxValue(temp[3])
            temp[0] = temp[0] ^ Rcon[int(i / Nk)]
        elif (Nk > 6 and i % Nk == 4):
            temp[0] = getSBoxValue(temp[0])
            temp[1] = getSBoxValue(temp[1])
            temp[2] = getSBoxValue(temp[2])
            temp[3] = getSBoxValue(temp[3])


        RoundKey.append(RoundKey[(i - Nk) * 4 + 0] ^ temp[0])
        RoundKey.append(RoundKey[(i - Nk) * 4 + 1] ^ temp[1])
        RoundKey.append(RoundKey[(i - Nk) * 4 + 2] ^ temp[2])
        RoundKey.append(RoundKey[(i - Nk) * 4 + 3] ^ temp[3])


        i = i + 1

def AddRoundKey(roundVar):
    for i in range(4):
        for j in range(4):
            state[j][i] ^= RoundKey[(roundVar * Nb * 4) + (i * Nb + j)]


def InvSubBytes():
    for i in range(4):
        for j in range(4):
            state[i][j] = getSBoxInvert(state[i][j] % 256)


def InvShiftRows():
    temp = state[1][3]
    state[1][3] = state[1][2]
    state[1][2] = state[1][1]
    state[1][1] = state[1][0]
    state[1][0] = temp
    temp = state[2][0]
    state[2][0] = state[2][2]
    state[2][2] = temp
    temp = state[2][1]
    state[2][1] = state[2][3]
    state[2][3] = temp
    temp = state[3][0]
    state[3][0] = state[3][1]
    state[3][1] = state[3][2]
    state[3][2] = state[3][3]
    state[3][3] = temp


def xtime(x):
    return ((x << 1) ^ (((x >> 7) & 1) * 0x1b))


def Multiply(x, y):
    return (((y & 1) * x) ^ ((y >> 1 & 1) * xtime(x)) ^ ((y >> 2 & 1) * xtime(xtime(x))) ^ (
                (y >> 3 & 1) * xtime(xtime(xtime(x)))) ^ ((y >> 4 & 1) * xtime(xtime(xtime(xtime(x))))))


def InvMixColumns():
    for i in range(4):
        a = state[0][i]
        b = state[1][i]
        c = state[2][i]
        d = state[3][i]
        state[0][i] = Multiply(a, 0x0e) ^ Multiply(b, 0x0b) ^ Multiply(c, 0x0d) ^ Multiply(d, 0x09)
        state[1][i] = Multiply(a, 0x09) ^ Multiply(b, 0x0e) ^ Multiply(c, 0x0b) ^ Multiply(d, 0x0d)
        state[2][i] = Multiply(a, 0x0d) ^ Multiply(b, 0x09) ^ Multiply(c, 0x0e) ^ Multiply(d, 0x0b)
        state[3][i] = Multiply(a, 0x0b) ^ Multiply(b, 0x0d) ^ Multiply(c, 0x09) ^ Multiply(d, 0x0e)


def InvCipher():
    for i in range(4):
        for j in range(4):
            state[j][i] = inn[i * 4 + j]
    AddRoundKey(Nr)
    for RV in reversed(range(Nr)):
        if (RV == 0):
            break;
        InvShiftRows()
        InvSubBytes()
        AddRoundKey(RV)
        InvMixColumns()
    InvShiftRows()
    InvSubBytes()
    AddRoundKey(0)
    for i in range(4):
        for j in range(4):
            out.append(state[j][i])


while(Nr!=128 and Nr!=192 and Nr!=256):
    print("Enter the length of Key (128, 192 or 256 only): ")
    Nr = int(input(""))
#Nr = 128;
Nk = Nr / 32
Nr = Nk + 6
Nr = int(Nr)
Nk = int(Nk)

AESKEY = list(map(str, input("Enter AES KEY:\n").split()))
Msg = list(map(str, input("Enter the message you would like to decrypt:\n").split()))

for i in range(Nk*4):
    Key.append(hex(int(AESKEY[i], 16)))
    Key[i] = int(Key[i], 16)
    inn.append(hex(int(Msg[i],16)))
    inn[i] = int(Msg[i], 16)

KeyExpansion()
InvCipher()
for o in out:
    print(hex(o), end= ' ')
